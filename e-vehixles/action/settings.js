...
// Save working hours
export async function saveWorkingHour(WorkingHour) {
  try {
    const { userId } = await auth();
    if (!userId) throw new Error("Unauthorized");
    ...
    if (!user || (user.role !== "ADMIN" && user.role !== "DEALER")) {
      throw new Error("Unauthorized: Admin or Dealer access required");
    }

    // Ensure dealership exists
    let dealership = await db.dealershipInfo.findFirst({
      where: {
        userId: user.id, // optional if you want per-dealer dealership
      },
    });
